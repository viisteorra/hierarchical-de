...
class SNLikelihood:
    ...
    def chi2(
        ...
        delta = self.data.mu - mu_model
        solved = cho_solve(self._cho, delta, check_finite=False)
    ...
    def fit_h0(
        ...
        """Experiment 1: lock w and Ω_DE, fit only H0."""

        def objective(H0: float) -> float:
            return self.chi2(float(H0), w=w, omega_de=omega_de)
